...
7. **Linear MCP (optional).** If the project has configured Linear (`.cursor/mcp.json` from `.cursor/mcp.json.example`) and tracking is agreed, follow `skills/linear-sync.md` after human approval: mirror the feature into Linear, list scenario IDs in the issue description, and fill the **Linear** column in `validation.md`. If Linear is not configured, skip this step.
